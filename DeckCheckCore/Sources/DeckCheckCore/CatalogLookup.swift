...
/// bundled SQLite snapshot (spec §3.1); tests back it with an in-memory fake.
/// Keeping resolution behind a protocol is what lets the whole core be unit-tested
/// without a database.
public protocol CatalogLookup {
    /// Printings matching a TCG Live set code + collector number (spec §6 step 2.1).
    ...
    /// "/191" set-pinning path used when the set code is unreadable (spec §3.2, §6).
    ...
    /// user's sheet (in-browser gap-check, §7.4 PART 2). A protocol *requirement* (not
